...
def printa(request):
    ...
    io7a = Coleta.objects.values_list('io7', flat=True).filter(id_transdutor=1).order_by('-id')[1]
    io7b = Coleta.objects.values_list('io7', flat=True).filter(id_transdutor=1).order_by('-id')[2]
